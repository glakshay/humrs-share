...
class Thruster(object):
    ...
    * `axis` (*type:* `numpy.array`): Axis of rotation of the thruster.
    ...
    def tam_update(self, axis=DEFAULT_AXIS):

        if self._pos is not None and self._orientation is not None:
            #self._pos = pos
            #self._orientation = orientation
            # compute contribution to configuration matrix of this thruster
            thrust_body = transformations.quaternion_matrix(self._orientation).dot(
                axis.transpose())[0:3]
            torque_body = numpy.cross(self._pos, thrust_body)
            self._force_dist = numpy.hstack((
                thrust_body, torque_body)).transpose()
