...
class ThrusterManager:
    ...
    def update_tam(self, recalculate=False):
        ...
        print('max_thrusters ' + str(self.MAX_THRUSTERS))
        ...
        for i in range(self.MAX_THRUSTERS):
            ...
            try:
                ...
                #print("pos " + str(listener.lookupTransform(base)))
                #print("pos1 " + str(pos))
                #a = [pos, quat]
                #print("pos and quat")
                #print(a)
    ...
    def dynamic_thruster_input(self, pos, orientation):
        #pass
        #self.thruster[i].pose = [x,y,z]
        #self.thruster[i].orientation= [x,y,z,w]
        #self.thrusters._pose = pose
        #self.thrusters._quat= quat
        #self.thrusters = tam_update(pose, quat)
        for i in range(self.n_thrusters):
            self.thrusters[i]._pos = pos[i]
            self.thrusters[i]._orientation = orientation[i]
            self.thrusters[i].tam_update()


        for j in range(self.n_thrusters):
            self.configuration_matrix[:, j] = self.thrusters[j].tam_column
    ...
    def command_thrusters(self):
        ...
        for i in range(self.n_thrusters):
            #self.thrusters[i].publish_command(1)
            self.thrusters[i].publish_command(self.thrust[i])

            #print("self thrust:" + str(self.thrust[i]) + "\n")

            #print("self thrust:" + str(self.thrust[i]) + "\n")
            #print("command thrusters:" + str(self.thrusters[i].publish_command(self.thrust[i])) + "\n")
